...
Remember to toggle the `Show Hidden Files` option in the `Files` pane to see all
files, including hidden ones.

In \@ref(fig:init-proj), except the `R` folder which you may have already known
the usage of it in [R-package-style file structure], the other three are:

- `ReprodBES.Rproj`: the RStudio project file which contains project-specific
  RStudio settings:
```{r, echo = FALSE}
readLines(with_project(projpath, proj_path("ReprodBES.Rproj")))
```
...
with_project(projpath, use_description())
```
...
with_project(projpath, use_mit_license("Hongyuan Jia"))
```
...
with_project(projpath, use_readme_md())
```
