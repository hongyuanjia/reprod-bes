...
analyses using the {eplusr} package. It is based on the {eplusr}
...
The main focuses of this chapter are to demonstrate:

1. parametric models creation by applying measures using {eplusr} and
2. data analyses of simulation outputs using {tidyverse}
...
We can use `eplusr::eplus_config()` to get the metadata of EnergyPlus installation:

```{r eplus-config}
eplus_config(9.4)
```

Once having EnergyPlus installation directory, we can get the path of the model
and weather input using the code below:
...
We can copy them into the `data-raw` directory we just created by running:
...
### `R` and `analysis` folder

First, let's create an `R` folder and inside create a `functions.R` script to
store the functions we will use to modify models and extract simulation outputs.
This can be easily done using `usethis:: use_r()` function:

```{r use-r-fun}
use_r("functions")
```

Inside `R/functions.R`, let's create a function called `preprocess_idf()` to do
some processing before we actually call the model for simulation, including:

1. Modify the `SimulationControl` object to use weather file for simulations.
   This is because by default the DOE Medium Office Building model only runs
...
```{r preprocess-idf, sink = here("R/functions.R")}
...
Then, let's create another folder called `analysis` using
`usethis::use_directory("analysis") `. This folder will be used to store the code that
calls functions in the `R/functions.R` to perform simulation and data analysis.

```{r}
use_directory("analysis")
```

After that, we create an R script called `run.R` under `analysis/` folder. You
can do that directly in RStudio, or by running the code blow:

```{r}
edit_file(here("analysis/run.R"))
```

Copy the code blow to `analysis/run/R` and run it by clicking
![](images/btn_source.png){#id width=10% style="vertical-align:middle;width:10%" display=inline-block}
or pressing `Ctrl + Shift + Enter` (`Cmd + Shift + Enter` on macOS). What the code below does is:

1. Source the functions we created inside `R/functions.R`
...
1. Pre-process the input model using `preprocess_idf()`
...
```{r init-job, out.lines = 10, sink = here("analysis/run.R")}
# load helper functions defined
source(here("R/functions.R"))

# specify model and weather file locations
...
# read and pre-process the input model
...
We can define a function `read_end_use()` in `R/functions.R` that takes a
simulation job, extract end uses and building area, and calculate the
electricity per area in kWh/m^2 in each category:

```{r read-end-use, sink = here("R/functions.R"), sink.append = TRUE}
...
```{r plot-end-use, sink = here("R/functions.R"), sink.append = TRUE}
...
Here, in `analysis/run.R`, we use the same model and weather file used in the
previous simulation.

```{r init-parm, sink = here("analysis/run.R"), sink.append = TRUE}
path_idf <- here("data-raw/RefBldgMediumOfficeNew2004_Chicago.idf")
path_epw <- here("data-raw/USA_IL_Chicago-OHare.Intl.AP.725300_TMY3.epw")
...
Since measures are functions that may be reused many times, it is better to save
them in `R/functions.R`
...
```{r set-lpd, sink = here("R/functions.R"), sink.append = TRUE}
...
In `R/functions.R`, we define a measure to modify the off-work schedule values
of plug loads by multiplying a specified reduction faction value.

```{r set-nightplug, sink = here("R/functions.R"), sink.append = TRUE}
...
`R/functions.R`, in order to pass it to the `$apply_measure()` method in next
step.

```{r comb-mea, sink = here("R/functions.R"), sink.append = TRUE}
...
models. In `analysis/run.R`, we add:

```{r apply-mea, sink = here("analysis/run.R"), sink.append = TRUE}
...
In `analysis/run.R`, we run:

```{r, eval = FALSE, sink = here("analysis/run.R"), sink.append = TRUE}
...
```{r param-end-use, sink = here("analysis/run.R"), sink.append = TRUE}
...
```{r cal-savings, sink = here("analysis/run.R"), sink.append = TRUE}
...
```{r savings, fig.width = 8, height = 3, fig.cap = "Energy savings of various lighting technologies and plug loads control strategies", sink = here("analysis/run.R"), sink.append = TRUE}
end_uses_savings %>%
