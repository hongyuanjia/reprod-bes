...
```{r tar-edit, echo = FALSE}
withr::with_dir(projpath, targets::tar_edit())
```

```{r, eval = FALSE, sink = here("_targets.R"), sink.append = TRUE}
library(targets)
library(tarchetypes)
...
list(
    ...
    tar_target(job, {
        ...
        idf$save(path("sim", path_file(idf$path())), overwrite = TRUE)
        idf$run(path_epw)
    }),
